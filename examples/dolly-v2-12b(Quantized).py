...
# Save the quantized weights and tokenizer to a temp location - we would package these with MLflow model
snapshot_location = "/root/.cache/quantized_model"
tokenizer.save_pretrained(snapshot_location)
quantized_model.save_pretrained(snapshot_location)
...
# Create a MLflow custom pyfunc class
...
input_example = pd.DataFrame(
    {"message": ["what is ML?"], "temperature": [0.8], "max_tokens": [250]}
)
...
result = mlflow.register_model("runs:/" + run.info.run_id + "/model", "dolly-v2-12b")
...
# In case of a GPU memory error, use the following code snippet to clear the memory (Might have to run this multiple times):
# import gc
# del loaded_model
...
input_example = pd.DataFrame(
    {"message": ["what is ML?"], "temperature": [0.5], "max_tokens": [100]}
)
